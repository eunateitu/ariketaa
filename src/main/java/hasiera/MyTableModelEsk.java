...
public class MyTableModelEsk extends AbstractTableModel {
	
	private Vector<Lag> data = new Vector<Lag>(); // Lag en bektorea
	private Vector<String> columnNames = new Vector<String>(); //zutabe izenen bektorea

	
	public MyTableModelEsk() {
		kargatu();
	}
...
//		}
	...
	public void addRow(Lag lag){ //errenkada bat gehitzeko
		data.add(lag);
		fireTableStructureChanged(); // --> Taularen egitura aldatzen denean jarri behar den komandoa
	}
	
	public void ezabatu(int hautatutakoa){ // errenkada bat ezabatzeko errenkada zein lekutan dagoen pasatuta
		data.remove(hautatutakoa);
		fireTableStructureChanged();
		}
	
	public int getRowCount() { // taulak zenbat errenkada dituen bueltatzen du
		return data.size();
	}
	
	public void hasieratuZutabeIzenak(){ //grafikan taularen zutabeei izenak jartzeko
	...
	public Lag errenkadaAtera(int errenkada){ // errenkadaren datuak ateratzeko
